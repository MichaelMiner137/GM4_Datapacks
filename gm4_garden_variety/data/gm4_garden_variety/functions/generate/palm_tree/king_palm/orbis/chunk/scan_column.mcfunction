...
# run from gm4_garden_variety:chunk/generate

scoreboard players remove structure_height gm4_count 1

# succeed if grass below
execute if block ~ ~-1 ~ minecraft:sand positioned ~ ~2 ~ if predicate gm4_fruiting_trees:3x3_clear positioned ~ ~-2 ~ run function gm4_garden_variety:generate/palm_tree/king_palm/orbis/chunk/spawn_tree

# next height
execute if block ~ ~-1 ~ #gm4_fruiting_trees:tree_replaceable if score structure_height gm4_count matches 64.. positioned ~ ~-1 ~ run function gm4_garden_variety:generate/palm_tree/king_palm/orbis/chunk/scan_column
