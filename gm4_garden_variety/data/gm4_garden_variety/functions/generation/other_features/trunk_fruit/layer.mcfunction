...
# place fruit
execute at @s align xyz positioned ~.5 ~.5 ~.5 if block ^ ^ ^1 air if block ^ ^-1 ^1 air positioned ^ ^ ^1 run function gm4_garden_variety:generation/other_features/trunk_fruit/place
execute at @s run tp @s ~ ~ ~ ~90 ~

# loop
scoreboard players remove trunk_fruit_layer_loop gm4_gv_gen_data 1
execute if score trunk_fruit_layer_loop gm4_gv_gen_data matches 1.. if score fruit_leaves gm4_gv_gen_data matches 1.. at @s run function gm4_garden_variety:generation/other_features/trunk_fruit/layer
