...
scoreboard players operation #identifier gm4_gv_pointer = $pointer.fill.identifier gm4_gv_component

# check if valid
execute if score #builder.length.current gm4_gv_component >= $pointer.fill.start gm4_gv_component if score #builder.length.current gm4_gv_component <= $pointer.fill.stop gm4_gv_component positioned ^ ^ ^1 run function gm4_garden_variety:generation/pointer/create_pointer
